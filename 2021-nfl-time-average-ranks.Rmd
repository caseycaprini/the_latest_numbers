...
df <- weekly_2021 %>%
  ...
  summarize(metal = median(mean_point_diff),
            ...
            mad = mad(mean_point_diff, constant = 1))
...
df %>%
  ggplot(aes(x = metal,
             y = reorder(factor(team), metal + mad))) +
  geom_linerange(aes(xmin = metal - mad,
                     xmax = metal + mad,
                     color = team)) +
  ...
  labs(title = "2021 Time Average Lead Distribution",
       subtitle = "Median Time Average Lead +/- MAD",
