...
game_results <- nfl_reg_season_summary(year = 2021) %>%
  ...
  mutate(`AVG +/-` = `Season Point Diff`/games_played) %>%
  select(team, RECORD, `AVG +/-`)
...
summ_tbl %>%
  select(team, team_logo_espn, METAL, TALE, `TALE SOS`) %>%
  arrange(desc(METAL)) %>%
  left_join(game_results,
            by = c("team" = "team")) %>%
  mutate(RANK = row_number(desc(METAL)),
         METAL = round(METAL, 1),
         TALE = round(TALE, 1),
         `TALE SOS` = round(`TALE SOS`, 1),
         `AVG +/-` = round(`AVG +/-`, 1)) %>%
  rename(TEAM = team,
         ` ` = team_logo_espn) %>%
  select(TEAM,
...
* <b>TALE</b>: Time Average Lead Evaluation
  * Average of time averaged leads for each game played by a team, adjusted for opponent quality (TALE SOS).
  * TALE - TALE SOS = raw time average lead for regular season games played
* <b>TALE SOS</b>: Time Average Lead Evaluation Strength of Schedule
  * Average of opponents time average leads against other NFL teams
* <b>AVG +/-</b>: Aggregate point differential, on per game basis
