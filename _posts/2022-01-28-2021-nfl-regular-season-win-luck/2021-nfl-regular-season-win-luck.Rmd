...
df_weekly = df_weekly %>%
  rename(tal = mean_point_diff) %>%
  mutate(exp_wins = ifelse(home_away == "home",
                           exp(home_int_coef + home_tal_coef*tal)/(1 + exp(home_int_coef + home_tal_coef*tal)),
                           exp(away_int_coef + away_tal_coef*tal)/(1 + exp(away_int_coef + away_tal_coef*tal)))
  )
...
```{r}
knitr::kable(luckiest_wins, format = "html", digits = 1)
```
...
```{r}
knitr::kable(exp_results %>% select(-games),
             format = "html",
             digits = 1)
```
