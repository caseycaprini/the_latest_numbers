...
For this analysis, I will use the tidyverse, packages from the nflverse, and lme4 for the multilevel model.
...
While penalty assessment is a bit arbitrary in the NFL, a lot of penalty yardage is earned by poor technique. Therefore, I'm considering net penalty yardage the same as yards gained or lost conventionally, irrespective of whether the down is replayed. This is a bit like preferring plate appearances for most analysis rather than at bats (since at bats exclude walks, scorer assessed sacrifices, etc.).
...
I don't discount the role of "hidden yardage" or the value of special teams. For this effort, however, I'm confining the analysis to the offensive and defensive units, since they are on the field for most snaps. I'm also throwing out the low hanging fruit like spikes and kneels where the purpose of the down is not gain yardage.
...
## The Advantage of a Multilevel Approach

Multilevel models have a number of advantages over simpler regression models. Importantly, multilevel models can make more realistic assumptions about the independence of observations in the data set.

In the NFL yards per play context, it's obvious that the quality of the defense faced matters when assessing the quality of an offense: gaining 5 yards/play against the league's best defense is better than gaining 5 yards/play against the league's worst defense.  Simple averages and linear regression models, however, do not recognize this and value the 5 yards/play the same, regardless of opponent (the independence assumption).

Random intercepts models are generally used as a baseline to compare more complicated multilevel models against. However, I'm going to use a random intercepts model as a method of averaging that recognizes opponent quality.
